 ...
 */
 ...
 // Wear calculation
        ...
        $wear = $baseWear * $wearMult * (float)$acqDefaults['wear_mult'] * $deltaHours;
        $wear *= (float)($hseBonus['degrade_mult'] ?? 1.0);

        if (!empty($lastMaintenanceAt)) {
            try {
                $maintAgeHours = max(0.0, (time() - strtotime((string)$lastMaintenanceAt)) / 3600);
                if ($maintAgeHours >= 240.0) {
                    $wear *= 1.60;
                } elseif ($maintAgeHours >= 144.0) {
                    $wear *= 1.35;
                } elseif ($maintAgeHours >= 72.0) {
                    $wear *= 1.15;
                }
            } catch (Throwable $e) {
