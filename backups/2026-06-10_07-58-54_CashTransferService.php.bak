...
class CashTransferService
{
    ...
    private function doTransfer(float $amount, string $fromPool, string $toPool): array
    {
        ...
        try {
            ...
            $lockedBalance = (float)$lockStmt->fetchColumn();
        ...
        // 4) Audit trail w bank_transactions (bez ruszania salda).
        // 4) Audit trail in bank_transactions (without touching the balance).
