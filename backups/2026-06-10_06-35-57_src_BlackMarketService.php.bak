 ...
 */
 ...
 // PL: Nie zabieraj wiecej niz gracz ma po transakcji.
            ...
            // Wiarygodnosc firmy: wykrycie czarnego rynku (po commit, poza transakcja).
            // Company credibility: black market detection (after commit, outside the transaction).
            if ($detected) {
                try {
                    (new CompanyCredibilityService($this->db))
                        ->applyEvent($playerId, 'black_market_detected');
                } catch (Throwable $e) {
                    if (class_exists('GameLog', false)) {
                        GameLog::error('BlackMarketService', 'credibility hook FAILED', $e, ['player_id' => $playerId]);
                    }
                }
            }
