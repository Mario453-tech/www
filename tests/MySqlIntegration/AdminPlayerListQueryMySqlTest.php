...
final class AdminPlayerListQueryMySqlTest extends TestCase
{
    public function testDateFiltersAndOrderingOnMySql(): void
    {
        ...
        $db->exec('CREATE TEMPORARY TABLE players (id INT, email VARCHAR(255), cash DECIMAL(15,2), status VARCHAR(30), last_login_at DATETIME, created_at DATETIME)');
        ...
        foreach ([
            [1, 'active', '2026-09-22 23:59:59', '2026-09-20 12:00:00'],
            [2, 'active', '2026-09-23 00:00:00', '2026-09-22 23:59:59'],
            [3, 'active', '2026-09-23 23:59:59', '2026-09-23 00:00:00'],
            [4, 'active', '2026-09-24 00:00:00', '2026-09-23 23:59:59'],
            [5, 'bankrupt', '2026-09-23 12:00:00', '2026-09-24 00:00:00'],
            [6, 'active', null, '2026-09-23 12:00:00'],
        ] as [$id, $status, $login, $registered]) {
            $insert->execute([$id, 'player' . $id . '@example.test', $status, $login, $registered]);
        }
        ...
        self::assertSame([4, 3, 6], $ids(['registered_from' => '2026-09-23', 'registered_to' => '2026-09-23']));
        self::assertSame([3], $ids(['registered_from' => '2026-09-23', 'registered_to' => '2026-09-23', 'login_from' => '2026-09-23', 'login_to' => '2026-09-23']));
        self::assertSame([4, 3, 5, 6], $ids(['registered_from' => '2026-09-23']));
        self::assertSame([2, 1], $ids(['registered_to' => '2026-09-22']));
        self::assertSame('2026-09-23 00:00:00', AdminPlayerListQuery::fetch($db, ['registered_from' => '2026-09-23', 'registered_to' => '2026-09-23'])[1]['created_at']);
