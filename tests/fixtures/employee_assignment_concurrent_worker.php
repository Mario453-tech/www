...
if ($argc < 7) {
    fwrite(
        STDERR,
        "Usage: php employee_assignment_concurrent_worker.php <playerId> <staffId> <hubId> <allocation> <readyFile> <gateFile>\n"
    );
    exit(2);
}
...
try {
    ...
    if (!is_file($gateFile)) {
        throw new RuntimeException('Concurrent employee assignment gate timeout.');
    }

    $service = new EmployeeAssignmentService($db);
    $result = $service->assignToHub(
