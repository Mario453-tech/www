...
final class RoadTransportServiceTest extends SqliteIntegrationTestCase
{
 ...
 //
 // applyTripIncidents mnozniki ochrony na wagach incydentow
 //

 /** @return array{float, float, list<array<string, mixed>>} */
    private function callApplyTripIncidents(array $protMults, int $trips = 2000): array
    {
        $ref = new ReflectionMethod(RoadTransportService::class, 'applyTripIncidents');
        $ref->setAccessible(true);
 // riskMult ekstremalny -> szansa bazowa przy capie 0.95 (przed skala ochrony)
        return $ref->invoke($this->svc, 1000.0, $trips, 99999.0, 1, 2, ['failure_reduction' => 1.0], $protMults);
    }

    public function testProtectionZeroMultEliminatesProtectedIncidentTypes(): void
    {
 // Mnoznik 0 dla theft/raid/sabotage: incydenty wylacznie accident/route_block.
 // Zero multiplier for theft/raid/sabotage: incidents only accident/route_block.
        [, , $incidents] = $this->callApplyTripIncidents([
            'theft' => 0.0, 'raid' => 0.0, 'sabotage' => 0.0,
        ]);

        $this->assertNotEmpty($incidents, 'Przy capie 0.95*0.5 i 2000 kursow incydenty musza wystapic.');
        foreach ($incidents as $inc) {
            $this->assertContains($inc['type'], ['accident', 'route_block'],
                'Typy chronione mnoznikiem 0 nie moga wystapic.');
        }
    }

    public function testProtectionScalesTotalChanceNotUnprotectedTypes(): void
    {
 // Wagi (3,2,4,1,2): zerowanie theft/raid/sabotage -> sum(w')/sum(w) = 6/12 = 0.5.
 // Laczna liczba incydentow spada ok. polowe wzgledem braku ochrony.
 // Weights (3,2,4,1,2): zeroing theft/raid/sabotage -> chance scale 0.5.
 // Total incident count drops by about half versus no protection.
        mt_srand(4242);
        [, , $unprotected] = $this->callApplyTripIncidents([]);
        mt_srand(4242);
        [, , $protected] = $this->callApplyTripIncidents([
            'theft' => 0.0, 'raid' => 0.0, 'sabotage' => 0.0,
        ]);

        $this->assertGreaterThan(1500, count($unprotected), 'Bez ochrony: cap 0.95 -> ~1900/2000.');
        $this->assertEqualsWithDelta(0.5, count($protected) / count($unprotected), 0.1,
            'Ochrona polowi laczna szanse (skala 6/12).');

 // Liczba incydentow accident/route_block nie moze wzrosnac przez ochrone.
 // accident/route_block incident counts must not grow because of protection.
        $countByType = static function (array $incidents, string $type): int {
            return count(array_filter($incidents, static fn(array $i): bool => $i['type'] === $type));
        };
        foreach (['accident', 'route_block'] as $type) {
            $this->assertEqualsWithDelta(
                $countByType($unprotected, $type),
                $countByType($protected, $type),
                $countByType($unprotected, $type) * 0.25 + 10,
                "Bazowe prawdopodobienstwo typu {$type} zachowane."
            );
        }
    }
}
