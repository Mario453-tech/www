...
function adminFinanceBuildViewData(PDO $db, FinanceService $finSvc, FinancePolicyService $policySvc, int $hours, array $config, array $mults): array
{
    ...
    $playerList = [];
    try {
        $playerList = $db->query("SELECT id, company FROM players WHERE status != 'bankrupt' ORDER BY company ASC")->fetchAll(PDO::FETCH_ASSOC);
    } catch (Throwable $e) {
