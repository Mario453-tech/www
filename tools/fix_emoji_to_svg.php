...
const KNOWN_EMOJI = [
    "\xE2\x9A\x99\xEF\xB8\x8F" => 'gear',          // ⚙️
    "\xE2\x9A\x99"             => 'gear',           // ⚙
    ...
    "\xE2\xAC\x86\xEF\xB8\x8F" => 'arrow-up',      // ⬆️
    "\xE2\xAC\x86"             => 'arrow-up',       // ⬆
    "\xE2\x8F\xB3"             => 'hourglass',      // ⏳
    "\xE2\x8F\xB1"             => 'hourglass',      // ⏱
    "\xE2\x9A\xA0\xEF\xB8\x8F" => 'warning',       // ⚠️
    "\xE2\x9A\xA0"             => 'warning',        // ⚠
    "\xF0\x9F\x8C\xBF"         => 'leaf',           // 🌿
    "\xE2\x9A\xA1\xEF\xB8\x8F" => 'lightning',     // ⚡️
    "\xE2\x9A\xA1"             => 'lightning',      // ⚡
    "\xF0\x9F\x94\x84"         => 'refresh',        // 🔄
    "\xF0\x9F\x97\x91\xEF\xB8\x8F" => 'trash',     // 🗑️
    ...
    "\xE2\x9D\x8C"             => 'warning',        // ❌
    "\xE2\xAF\xB8\xEF\xB8\x8F" => 'pause',         // ⏸️
    "\xE2\xAF\xB8"             => 'pause',          // ⏸
    "\xF0\x9F\x9B\xA2\xEF\xB8\x8F" => 'oil-drop',  // 🛢️
    "\xF0\x9F\x9B\xA2"         => 'oil-drop',       // 🛢
    "\xE2\x9B\xAD\xEF\xB8\x8F" => 'oil-drop',      // ⛽️
 ...
 * so Polish letters (a, e, o...) are NOT touched.
 *
 ...
 * - T_INLINE_HTML (HTML outside PHP tags): emoji => echo tag wgIco('name')
 * - T_CONSTANT_ENCAPSED_STRING (PHP string literal): emoji => wgIco('name') . '...'
 */
function replaceEmojiInFile(string $content, array $emojiMap): string
{
    ...
    foreach ($tokens as $token) {
        ...
        switch ($type) {
            case T_INLINE_HTML:
 // HTML context: emoji becomes echo tag wgIco('name')
...
/**
 * Replaces emoji inside a PHP single- or double-quoted string token.
 * Input: '🌿 Eco'
 * Output: wgIco('leaf') . ' Eco'
 *
 * Input: '❓'
 * Output: wgIco('question')
 */
function replaceInPhpString(string $text, array $emojiMap): string
{
 // Check if any emoji is present at all (fast path)
    $found = false;
    foreach ($emojiMap as $emoji => $_) {
        if (str_contains($text, $emoji)) { $found = true; break; }
    }
    if (!$found) return $text;

    $quote = $text[0];           // ' or "
    $inner = substr($text, 1, -1); // strip surrounding quotes

 // Build regex alternation of all emoji sequences (sorted by length DESC already)
    $alts    = array_map('preg_quote', array_keys($emojiMap));
    $pattern = '/(' . implode('|', $alts) . ')/u';

    $parts = preg_split($pattern, $inner, -1, PREG_SPLIT_DELIM_CAPTURE) ?? [$inner];

    $segments = [];
    foreach ($parts as $part) {
        if ($part === '') continue;
        if (isset($emojiMap[$part])) {
            $segments[] = "wgIco('{$emojiMap[$part]}')";
        } else {
            $segments[] = $quote . $part . $quote;
        }
    }

    if (empty($segments)) return $text;
