 ...
 // 
    ...
    function renderLocList(locs, containerId) {
        ...
        el.innerHTML = sorted.map(function(loc) {
            ...
            // Brief §5: badge zależy od pełnego permit_status, nie tylko has_permit.
            // Brief §5: badge depends on full permit_status, not just has_permit.
            var ps = r ? (r.permit_status || (r.has_permit ? 'active' : 'none')) : 'active';
            var needsPermit = !isMine && !isOccupied && ps !== 'active';
            var badge      = isMine
    ...
    function showRegion(regionId) {
        ...
        // Brief §6.1: baner aktywnego zezwolenia — gdy gracz ma zezwolenie na region.
        // Brief §6.1: active-permit banner — when the player holds a permit for the region.
        var srPermit = document.getElementById('sr-permit');
        if (srPermit) {
            var psActive = r.permit_status ? (r.permit_status === 'active') : (r.has_permit === true);
            if (psActive) {
                srPermit.className = 'sr-permit sr-permit--active';
                srPermit.innerHTML = mlt('permit_active_banner');
                srPermit.style.display = '';
            } else {
                srPermit.style.display = 'none';
                srPermit.innerHTML = '';
            }
        }
    ...
    function showLocation(locationId) {
        ...
        } else if (!r || r.permit_status !== 'active') {
            // Brief §5-§7, §14: 5 wariantów modalu zależnie od permit_status.
            // Brief §5-§7, §14: 5 modal variants depending on permit_status.
            var permitPs = r ? (r.permit_status || 'none') : 'none';
            buyHtml = buildPermitHtml(permitPs, r);
    ...
    // Brief §6.2: przycisk "Anuluj" w modalu braku zezwolenia — powrót do
    // panelu regionu (lub placeholder, jeśli brak aktywnego regionu).
    // Brief §6.2: "Cancel" button in the no-permit modal — back to the region
    // panel (or placeholder when no region is active).
    window.__mapBackToRegion = function () {
        if (currentRegionId) showRegion(currentRegionId); else showPanel('placeholder');
    };

    // ---- Pomocnicze funkcje modali zezwoleń / Permit modal helper functions ----

    // Brief §5: badge listy lokalizacji — różne dla każdego statusu.
    // Brief §5: location list badge — distinct per status.
    function permitBadge(ps) {
        switch (ps) {
            case 'pending':
            case 'delayed':
                return '<span class="loc-badge loc-badge--permit-pending">' + mlt('badge_permit_pending')     + '</span>';
            case 'no_decision':
                return '<span class="loc-badge loc-badge--permit-pending">' + mlt('badge_permit_no_decision') + '</span>';
            case 'refused':
    ...
    // Brief §6.2-§6.5 / §7.3 / §14.1-§14.4: treść sekcji zakupu zależna od statusu.
    ...
    function buildPermitHtml(ps, r) {
        ...
        if (ps === 'pending' || ps === 'delayed') {
            // Brief §6.3 / §14.3: "Wniosek w trakcie" / "Application in progress"
            var minLeft = r && r.permit_minutes_left != null ? r.permit_minutes_left : null;
            var timeStr = minLeft != null ? fmtMinutes(minLeft) : null;
            var bodyText = timeStr
                ? mlt('permit_pending_text').replace(':time', timeStr)
                : mlt('permit_pending_text_unknown');
            return '<div class="loc-permit-required loc-permit-required--pending">'
                + '<div class="loc-permit-title">' + mlt('permit_pending_title') + '</div>'
                + '<div class="loc-permit-text">'  + bodyText + '</div>'
                + '<div class="loc-permit-actions">' + closeBtn + goLegal + '</div>'
                + '</div>';
        }

        if (ps === 'no_decision') {
            // Brief §6.4 / §14.4: "Brak decyzji" / "No decision"
            return '<div class="loc-permit-required loc-permit-required--no-decision">'
                + '<div class="loc-permit-title">' + mlt('permit_no_decision_title') + '</div>'
                + '<div class="loc-permit-text">'  + mlt('permit_no_decision_text')  + '</div>'
                + '<div class="loc-permit-actions">' + closeBtn + goLegal + '</div>'
                + '</div>';
        }

        if (ps === 'refused') {
            // Brief §6.5: "Wniosek odrzucony" / "Application refused"
            var coolMin  = r && r.permit_cooldown_minutes != null ? r.permit_cooldown_minutes : null;
            var coolLine = coolMin != null
                ? '<div class="loc-permit-cooldown">' + mlt('permit_refused_cooldown').replace(':time', fmtMinutes(coolMin)) + '</div>'
                : '';
            return '<div class="loc-permit-required loc-permit-required--refused">'
                + '<div class="loc-permit-title">' + mlt('permit_refused_title') + '</div>'
                + '<div class="loc-permit-text">'  + mlt('permit_refused_text')  + '</div>'
                + coolLine
                + '<div class="loc-permit-actions">' + closeBtn + goLegal + '</div>'
                + '</div>';
        }
        ...
        if (ps === 'locked') {
            // Brief §7.3 / §14.2: "Region wysokiego ryzyka" / "High-risk region"
            var cap = r && r.permit_required_capital != null ? r.permit_required_capital : null;
            var capLine = cap != null
                ? '<div class="loc-permit-capital">' + mlt('permit_locked_capital').replace(':amount', fmt(cap)) + '</div>'
                : '';
            return '<div class="loc-permit-required loc-permit-required--locked">'
                + '<div class="loc-permit-title">' + mlt('permit_locked_title') + '</div>'
                + '<div class="loc-permit-text">'  + mlt('permit_locked_text')  + '</div>'
                + capLine
                + '<div class="loc-permit-actions">' + closeBtn + goLegal + '</div>'
                + '</div>';
        }
        ...
        return '<div class="loc-permit-required">'
            ...
            + '<div class="loc-permit-rows">'
            +     '<div class="loc-permit-row"><span class="loc-permit-lbl">' + mlt('permit_type_label') + '</span>'
            +         '<span class="loc-permit-val">' + mlt('permit_type_drilling') + '</span></div>'
            + '</div>'
            + '<div class="loc-permit-unlocks">' + mlt('permit_required_unlocks') + '</div>'
            + '<div class="loc-permit-actions">' + cancelBtn + goLegal + '</div>'
            + '</div>';
    }
