...
// O stou: centrum ~50% x, ~55% y (st jest bardziej wycentrowany)
// St okrgy: promie ~32% ekranu (wikszy st na nowym obrazie)
// Miejsca rozmieszczone rwnomiernie wok okrgu

const CX = 50;   // centrum X (%)
const CY = 52;   // centrum Y (%) - jeszcze wyej
const RX = 30;   // promie X (%)
const RY = 16;   // promie Y (%) - lekka perspektywa

// 10 miejsc - DOKADNIE dopasowane do rzeczywistych krzese na zdjciu
...
// 
// SYSTEM DYNAMICZNEGO TA
// 
// Koncepcja: Zamiast nakada sloty na statyczny obraz, 
...
// Stan obsadzonych stanowisk
const occupiedPositions = ['director', 'hr']; // dyrektor + HR ju obsadzone

// Funkcja zwracajca nazw pliku ta na podstawie obsadzonych stanowisk
function getBackgroundImage() {
  if (occupiedPositions.length === 1 && occupiedPositions[0] === 'director') {
...
function render() {
  container.innerHTML = '';
  const W = sceneEl.offsetWidth;
  const H = sceneEl.offsetHeight;

  seats.forEach(seat => {
    const rad = toRad(seat.angle - 90); // -90 eby 0 = gra
    const x = CX + RX * Math.cos(rad);
    const y = CY + RY * Math.sin(rad);

    const div = document.createElement('div');
    div.className = 'slot' +
      (seat.role === 'director' ? ' slot-director' : '') +
      (seat.role === 'hr'       ? ' slot-hr'       : '') +
      (seat.role === 'occupied' ? ' slot-occupied' : '');
    div.style.left = x + '%';
    div.style.top  = y + '%';

    const tooltipText = seat.tooltip ||
      (seat.role === 'director' ? 'Dyrektor  to Ty' :
       seat.role === 'occupied' ? seat.label + '  obsadzone' :
       seat.role === 'empty'    ? 'Stanowisko nieobsadzone. Brak dziau kadr.' :
       seat.label);

    const hrBadge = seat.role === 'hr'
      ? `<div class="hr-badge"> Wymagane</div>` : '';

    div.innerHTML = `
      <div class="slot-avatar" style="transform:scale(${seat.scale})">
        ${hrBadge}
        ${avatarSVG(
            seat.role === 'director' ? '#c8a84b' :
            seat.role === 'occupied' ? '#4ec97a' :
            seat.role === 'hr'       ? '#e8cc7a' : 'white',
            seat.role === 'director' ? 32 : 26
          )}
        <div class="slot-num">${seat.role === 'director' ? 'DYREKTOR' : seat.role === 'occupied' ? seat.label.toUpperCase() : seat.role === 'hr' ? 'HR' : '#' + String(seat.id).padStart(2,'0')}</div>
      </div>
...
// Inicjalizacja
updateBackground(); // Zaaduj odpowiednie to na podstawie occupiedPositions
render();
