 ...
 */
 ...
 // 'crediting' (transitional: trip computed but not yet confirmed in storage).
            $this->ensureRoadTripStatusEnum();
        }
    }

 /**
 * M3: Dodaje wartosci 'crediting' i 'delayed' do ENUM status w istniejacych tabelach (migracja).
 * M3: Adds 'crediting' and 'delayed' values to the status ENUM on existing tables (migration).
 */
    private function ensureRoadTripStatusEnum(): void
    {
        try {
 ...
 // Brak tabeli (pusty) -> nic nie rob. Juz zmigrowane (zawiera oba) -> nic nie rob.
 // No table (empty) -> nothing to do. Already migrated (contains both) -> nothing to do.
            if ($type === '') {
                return;
            }
            $needsCrediting = strpos($type, "'crediting'") === false;
            $needsDelayed   = strpos($type, "'delayed'")   === false;
            if ($needsCrediting || $needsDelayed) {
                $this->db->exec(
                    "ALTER TABLE well_road_trips
                        MODIFY COLUMN status
                        ENUM('in_transit','crediting','delivered','lost','delayed')
                        NOT NULL DEFAULT 'in_transit'"
                );
            }
        } catch (Throwable $e) {
            if (class_exists('GameLog', false)) {
                GameLog::error('RoadTransportService', 'ensureRoadTripStatusEnum FAILED', $e);
            }
        }
    }
 ...
 */
    public function dispatchTrips(
        ...
        // well_road_trips nie istnieje na SQLite / well_road_trips does not exist on SQLite
 ...
 */
 ...
 /** @var array<int, float> well_id => delivered bbl (basis for the second transport leg) */
        ...
        // Tabela well_road_trips istnieje tylko w MySQL — na SQLite nie ma kursow czasowych.
        // well_road_trips exists only in MySQL — time-based trips are not supported on SQLite.
        if ($this->db->getAttribute(PDO::ATTR_DRIVER_NAME) === 'sqlite') {
            return ['delivered_bbl' => 0.0, 'lost_bbl' => 0.0, 'completed_count' => 0, 'delivered_by_well' => []];
        }

 // M3 RECOVERY: ponownie skredytuj osierocone kursy 'crediting' z poprzedniego ticka.
 // Zapis magazynu i potwierdzenie dostawy sa atomowe (patrz PlayersSection::processPlayer),
 // wiec kurs nadal 'crediting' na poczatku ticka oznacza, ze magazyn NIE zostal zapisany —
 // ropa nigdy nie trafila do gracza i trzeba ja skredytowac ponownie. delivered_bbl oraz
 // incydenty sa juz utrwalone z pierwotnego ticka, wiec NIE losujemy i NIE logujemy ponownie.
 // M3 RECOVERY: re-credit orphaned 'crediting' trips from a crashed previous tick.
 // Storage write and delivery confirmation are atomic (see PlayersSection::processPlayer),
 // so a trip still 'crediting' at tick start means storage was NOT committed — the oil never
 // reached the player and must be credited again. delivered_bbl and incidents are already
 // persisted/logged from the original tick, so we DO NOT re-roll or re-log.
        try {
            $recoverStmt = $this->db->prepare(
                "SELECT id, well_id, delivered_bbl
                   FROM well_road_trips
                  WHERE player_id = ? AND status = 'crediting'
                  ORDER BY eta_at ASC"
            );
            $recoverStmt->execute([$playerId]);
            $orphans = $recoverStmt->fetchAll(PDO::FETCH_ASSOC);
        } catch (Throwable $e) {
            if (class_exists('GameLog', false)) {
        ...
        foreach ($orphans as $orphan) {
            ...
            $totalDelivered += $recBbl;
            $completed++;
            $wid = (int)$orphan['well_id'];
            $deliveredByWell[$wid] = ($deliveredByWell[$wid] ?? 0.0) + $recBbl;
            if (class_exists('GameLog', false)) {
                GameLog::warn('RoadTransportService', 'road_trip_recovered_after_crash', [
                    'trip_id' => (int)$orphan['id'], 'player_id' => $playerId, 'bbl' => round($recBbl, 4),
                ]);
            }
        }
 ...
 /** @var array<int, array<string,mixed>|null> well_id => aktywna ochrona / active protection */
        ...
        foreach ($trips as $trip) {
            ...
            // Kurs juz przeliczony i opozniony sabotazem: tylko finalizacja po uplywie ETA.
            // Trip already computed and delayed by sabotage: just finalize once ETA passes.
            if ((string)$trip['status'] === 'delayed') {
                ...
                try {
                    ...
                    // delayed and is reprocessed next tick without double-crediting.
                    $totalDelivered += $delivered;
                    $totalLost      += max(0.0, (float)$trip['volume_bbl'] - $delivered);
                    $completed++;
            ...
            [$delivered, $lost, $incidents, $delayMinutes] = $this->applyTripIncidents(
            ...
            // Sabotage with a delay effect: store the result, push ETA, finalize on a later tick.
            if ($delayMinutes > 0) {
                $deferOk = false;
                try {
                    $this->db->prepare(
                ...
                // Logs (protection + incidents) only after a confirmed write — on failure the trip
                // stays in_transit and everything is logged once on reprocessing.
                if ($deferOk) {
                    $this->logProtectionAndIncidents(
                        $protection, $activeProt, $playerId, $wid,
            ...
            try {
 // Faza 1: oznacz 'crediting' (NIE 'delivered') i zapisz policzony delivered_bbl.
 // Potwierdzenie na 'delivered' nastapi w confirmCreditedTrips() w tej samej
 // transakcji co zapis magazynu. arrived_at ustawiamy dopiero przy potwierdzeniu.
 // Phase 1: mark 'crediting' (NOT 'delivered') and store the computed delivered_bbl.
 // Confirmation to 'delivered' happens in confirmCreditedTrips() within the same
 // transaction as the storage write. arrived_at is set only at confirmation.
                $this->db->prepare(
                    "UPDATE well_road_trips
                ...
                // log failure cannot undo the delivered-volume accounting.
                $totalDelivered += $delivered;
                $totalLost      += $lost;
                $completed++;
                $deliveredByWell[$wid] = ($deliveredByWell[$wid] ?? 0.0) + $delivered;
 ...
 // Bug #2: If the DB status write failed, the trip stays 'in_transit' and will
 // be re-processed next tick. Skip accumulation to avoid double-crediting the player.
                continue;
            }
        }
 ...
 /**
 * M3: Potwierdza kursy 'crediting' gracza jako 'delivered'. MUSI byc wywolane w tej
 * samej transakcji co zapis magazynu, aby kredyt do magazynu i potwierdzenie dostawy
 * commitowaly sie atomowo. Rzuca wyjatek przy bledzie (celowo — aby wycofac transakcje).
 * M3: Confirms a player's 'crediting' trips as 'delivered'. MUST be called within the
 * same transaction as the storage write so that storage credit and delivery confirmation
 * commit atomically. Throws on error by design (to roll back the transaction).
 *
 * @return int liczba potwierdzonych kursow / number of confirmed trips
 */
    public function confirmCreditedTrips(int $playerId): int
    {
        $stmt = $this->db->prepare(
            "UPDATE well_road_trips
                SET status = 'delivered', arrived_at = NOW()
 ...
 */
    ...
    {
        // well_road_trips istnieje tylko w MySQL / well_road_trips exists only in MySQL
        if ($this->db->getAttribute(PDO::ATTR_DRIVER_NAME) === 'sqlite') {
            return [];
        }
    ...
    private function ensureRoadTripColumn(string $column, string $definition): void
    {
        try {
            ...
            if (!in_array($column, $allowedColumns, true) || !in_array($definition, $allowedDefinitions, true)) {
                GameLog::error('RoadTransportService', 'ensureRoadTripColumn BLOCKED — unknown column/definition', null, ['column' => $column]);
                return;
            }
 ...
 * @return array{float, float, list<array<string, mixed>>, int} delivered, lost, incidents, max delay minutes
 */
    private function applyTripIncidents(
        ...
        for ($i = 0; $i < $tripsCount; $i++) {
            ...
            if ($type === 'sabotage' && $sabotage !== null && $playerId > 0 && $wellId > 0) {
                ...
                $maxDelay = max($maxDelay, (int)($sabResult['delay_minutes'] ?? 0));
            }
 ...
 */
    ...
    /**
     * Loguje skutek kursu (ochrona + incydenty) po potwierdzonym zapisie do bazy.
     * Nie rzuca: blad logu nie moze cofnac rozliczenia kursu. Loguje takze incydenty
     * bez straty wolumenu (np. sabotaz typu czyste opoznienie, lost=0).
     * Logs the trip outcome (protection + incidents) after a confirmed DB write.
     * Never throws: a logging failure must not undo trip accounting. Also logs incidents
     * with no volume loss (e.g. pure-delay sabotage, lost=0).
     *
     * @param array<string, mixed>|null $activeProt
     * @param list<array<string, mixed>> $incidents
     */
    private function logProtectionAndIncidents(
        ...
        if ($protection !== null && $activeProt !== null) {
            try {
                ...
                if (class_exists('GameLog', false)) {
                    GameLog::error('RoadTransportService', 'logProtectionAndIncidents protection FAILED', $e, ['well_id' => $wellId]);
                }
            }
        }
        $this->logIncidents($wellId, $playerId, $tripsCount, $lost, $incidents);
    }
