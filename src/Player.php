...
class Player
{
 ...
 /** @return array<string, mixed>|false|null */
    ...
    {
        // Wzorzec $ownTx — metoda moze byc wywolana samodzielnie lub wewnatrz transakcji wywolujacego.
        ...
        try {
            if ($amount < 0) {
                // Atomowe odjecie — zapobiega zejsciu cash ponizej zera przy rownoczesnych requestach.
                // Atomic deduct — prevents cash going below zero under concurrent requests.
                $stmt = $this->db->prepare("
                    UPDATE players
                ...
                $stmt = $this->db->prepare("
                    ...
                    WHERE id = :id
                ");
                // Bugfix: execute() zwraca true nawet gdy gracz nie istnieje (rowCount=0).
                // Bugfix: execute() returns true even when player not found (rowCount=0); check rowCount instead.
                $stmt->execute([':amount' => $amount, ':id' => $this->playerId]);
                $ok = $stmt->rowCount() > 0;
            }
            if (class_exists('GameLog', false)) {
            ...
            // Operacja poboczna — blad nie powoduje rollback (zasada 6 / Rule 6: side ops in try/catch).
            // Side operation — failure must not trigger rollback (Rule 6).
