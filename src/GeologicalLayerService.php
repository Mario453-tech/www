 ...
 */
    public function switchLayer(int $wellId, int $playerId, int $targetLayerId): array
    {
        try {
            ...
            // Sprawdz bankructwo przed transakcja — updateCash nie sprawdza isBankrupt (Rule 10).
            // Check bankruptcy before transaction — updateCash does not check isBankrupt (Rule 10).
            $player = null;
            if ($cost > 0) {
                $player = new Player($playerId);
                if ($player->isBankrupt()) {
                    return ['success' => false, 'message' => $player->getBankruptcyBlockMessage()];
                }
            }

            // Wzorzec $ownTx — metoda moze byc wywolana samodzielnie lub wewnatrz transakcji wywolujacego (Rule 3/5).
            // $ownTx pattern — method may be called standalone or inside a caller's transaction (Rule 3/5).
            $ownTx = !$this->db->inTransaction();
            if ($ownTx) $this->db->beginTransaction();
            try {
                if ($cost > 0 && $player !== null) {
                    if (!$player->updateCash(-$cost, \FinancialTransactionService::TYPE_GEOLOGICAL_FEE, 'Oplata za zmiane warstwy geologicznej')) {
                ...
                // Filtruj po player_id — blokuje race condition przejecia odwiertu miedzy SELECT a UPDATE (Rule 1).
                // Filter by player_id — blocks well-seizure race between SELECT and UPDATE (Rule 1).
                $this->db->prepare("
                    UPDATE wells
                    SET active_layer_id      = ?,
                ...
                ")->execute([$targetLayerId, $switchUntil, $hours, $wellId, $playerId]);
