...
class SabotageSchema
{
    ...
    {
        // Add player_id index for getPlayerCooldownMap / listAttemptsForPlayer performance.
        // Dodaj indeks player_id dla wydajnosci getPlayerCooldownMap / listAttemptsForPlayer.
        try {
            $has = $db->query(
                "SELECT COUNT(*) FROM information_schema.STATISTICS
                  WHERE TABLE_SCHEMA = DATABASE()
                    AND TABLE_NAME = 'sabotage_attempts'
                    AND INDEX_NAME = 'idx_sabotage_attempt_player'"
            )->fetchColumn();
            if ((int)$has === 0) {
                $db->exec("ALTER TABLE sabotage_attempts ADD INDEX idx_sabotage_attempt_player (player_id)");
            }
        } catch (Throwable $e) {
            if (class_exists('GameLog', false)) {
                GameLog::error('SabotageSchema', 'migrateMysql: add player_id index FAILED', $e);
            }
        }
    }
