 ...
 */
 ...
 // Rozsadne minimum ceny budowy/ulepszenia per typ — hub NIGDY nie moze byc
 // za darmo, nawet gdy konfiguracja zwraca 0 lub puste (np. blad w panelu admina).
 // Wartosc poprawnie ustawiona (>0) jest zawsze uzywana bez zmian.
 // Sensible minimum build/upgrade cost per type — a hub must NEVER be free even
 // if config returns 0/empty (e.g. an admin-panel mistake). A properly set
 // value (>0) is always used as-is.
        $buildCostFloor = ['small' => 50000.0, 'medium' => 150000.0, 'large' => 400000.0][$hubType] ?? 50000.0;
        $upgradeCostFloor = ['small' => 30000.0, 'medium' => 80000.0, 'large' => 200000.0][$hubType] ?? 30000.0;

        $buildCost   = (float)($base['build_cost'] ?? 0);
        $upgradeCost = (float)($base['upgrade_cost'] ?? 0);
        if ($buildCost <= 0.0) {
            $buildCost = $buildCostFloor;
        }
        if ($upgradeCost <= 0.0) {
            $upgradeCost = $upgradeCostFloor;
        }
