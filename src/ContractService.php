...
class ContractService
{
     ...
     * bezpieczenstwa: allow_cancel, cancel_forfeit_deposit, allow_renegotiation itd.) sa
     * chronione przed nadpisaniem przez gracza.
     * Term keys a player may change via renegotiation. Security flags are protected from
     * being overwritten through renegotiation.
     */
    ...
    public function getAvailableOptions(int $playerId, string $targetType, string $context, float $referenceValue = 0.0): array
    {
        ...
        $marketPrice = $referenceValue > 0.0 ? $referenceValue : $this->currentMarketPrice();
    ...
    public function acceptContract(int $playerId, int $optionId, string $targetType, ?int $targetId, string $context): array
    {
        ...
        $contractReputationScore = $this->contractReputation->getScore($playerId);
        $locked = $this->lockedReason(
            $option,
            $this->credibility->getScore($playerId),
            $legalLevel,
            $terms,
            $contractReputationScore
        );
        if ($locked !== null) {
            return $this->result(false, 'requirements_' . $locked);
        }
        $securityDeposit = $this->calculateSecurityDeposit($terms, $this->currentMarketPrice(), $contractReputationScore);
        $now = $this->nowString();
        ...
        if ($securityDeposit >= FinancialTransactionService::MIN_AMOUNT) {
        ...
        try {
            ...
            if ($ownTx) {
                ...
                $savepoint = 'contract_accept_' . $playerId . '_' . $optionId;
            ...
            if ($securityDeposit >= FinancialTransactionService::MIN_AMOUNT && $this->combinedBalanceForUpdate($playerId) + 1e-9 < $securityDeposit) {
                $this->rollbackContractTransaction($ownTx, $savepoint);
                return $this->result(false, 'insufficient_deposit_funds');
            }
            $stmt = $this->db->prepare(
                "INSERT INTO player_contracts
                    (player_id, contract_option_id, target_type, target_id, context, buyer_name, contract_name,
                     status, total_bbl, delivered_bbl, missed_bbl, security_deposit, security_deposit_status,
                     next_delivery_at, starts_at, ends_at,
            ...
            $stmt->execute([
                ...
                $securityDeposit,
                $securityDeposit >= FinancialTransactionService::MIN_AMOUNT ? 'paid' : 'none',
                $next,
                $now,
                $ends,
                $termsJson,
            ...
            if ($securityDeposit >= FinancialTransactionService::MIN_AMOUNT) {
                $depositDebit = $this->fts->debitCombined(
                    $playerId,
                    $securityDeposit,
            ...
            $this->finishContractTransaction($ownTx, $savepoint);
    ...
    public function cancelContract(int $playerId, int $contractId): array
    {
        ...
        try {
            ...
            if ($ownTx) {
                ...
                $savepoint = 'contract_cancel_' . $contractId;
                $this->db->exec('SAVEPOINT ' . $savepoint);
            }
            ...
            if ((float)($terms['allow_cancel']['value'] ?? 1.0) <= 0.0) {
                $this->rollbackContractTransaction($ownTx, $savepoint);
                return $this->result(false, 'cancel_not_allowed');
            }
            $cancelPenalty = $this->calculateCancelPenalty($terms, $row, $this->currentMarketPrice());
            if ($cancelPenalty >= FinancialTransactionService::MIN_AMOUNT) {
                $penaltyDebit = $this->fts->debitCombined(
                    $playerId,
                    $cancelPenalty,
                ...
                if (!$penaltyDebit['success']) {
                    $this->rollbackContractTransaction($ownTx, $savepoint);
                    return $this->result(false, 'cancel_insufficient_funds');
                }
            }
            $this->db->prepare(
            ...
            )->execute([$now, $cancelPenalty, 'player_cancelled', $now, $contractId, $playerId]);
            $this->settleSecurityDeposit($row, 'cancelled', (float)$row['delivered_bbl'], $now);
            $blockMinutes = max(0, (int)round((float)($terms['cancel_blocks_new_contracts_minutes']['value'] ?? 0.0)));
            if ($blockMinutes > 0) {
                $this->contractReputation->blockNewContracts($playerId, $blockMinutes);
            }
            $this->contractReputation->onContractCancelled($playerId, $contractId);
            $this->logEvent($playerId, $contractId, (string)$row['target_type'], $row['target_id'] === null ? null : (int)$row['target_id'], (string)$row['context'], 'contract_cancelled', 'Contract cancelled by player', [
    ...
    private function processOneDueContract(array $contract, string $nowStr, float $marketPrice): array
    {
        ...
        try {
            ...
            if ($newStatus === 'completed') {
                $this->settleSecurityDeposit($contract, $newStatus, $newDeliveredBbl, $nowStr);
    ...
    /**
     * Renegocjuje warunki aktywnego kontraktu (tylko klucze z allowlisty RENEGOTIABLE_TERMS).
     * Renegotiates terms of an active contract (allowlisted keys only).
     *
     * @param array<string,float> $termOverrides  Mapa klucz => nowa wartosc liczbowa.
     * @return array<string,mixed>
     */
    public function renegotiateContract(int $playerId, int $contractId, array $termOverrides): array
    {
        ...
        try {
            ...
            if ($ownTx) {
                ...
                $savepoint = 'contract_reneg_' . $contractId;
                $this->db->exec('SAVEPOINT ' . $savepoint);
            }
            $row = $this->contractForUpdate($playerId, $contractId);
            if ($row === null) {
            ...
            $applied  = 0;
            foreach ($termOverrides as $key => $value) {
                // Tylko dozwolone klucze / Allowlist only - protects security flags.
                if (!in_array($key, self::RENEGOTIABLE_TERMS, true)) {
                    continue;
                }
                if (isset($terms[$key])) {
            ...
            $newTermsJson = json_encode($terms, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
            $this->db->prepare(
                "UPDATE player_contracts SET terms_json = ?, renegotiations_used = renegotiations_used + 1, last_renegotiated_at = ?, updated_at = ? WHERE id = ? AND player_id = ?"
            )->execute([$newTermsJson, $now, $now, $contractId, $playerId]);
            $this->db->prepare(
                "INSERT INTO contract_renegotiations (player_contract_id, player_id, old_terms_json, new_terms_json, message, created_at) VALUES (?, ?, ?, ?, '', ?)"
            )->execute([
                $contractId,
    ...
    private function finishContractTransaction(bool $ownTx, ?string $savepoint): void
    {
        if ($ownTx) {
            $this->db->commit();
            return;
    ...
    private function rollbackContractTransaction(bool $ownTx, ?string $savepoint): void
    {
    ...
    private function calculateSecurityDeposit(array $terms, float $marketPrice, int $contractReputation): float
    {
        $totalBbl = (float)($terms['total_bbl']['value'] ?? 0.0);
        $pricePerBbl = $this->calculatePrice($terms, max(0.0, $marketPrice));
        $pct = max(0.0, (float)($terms['security_deposit_pct']['value'] ?? 0.0));
        $fixed = max(0.0, (float)($terms['security_deposit_fixed']['value'] ?? 0.0));
        $deposit = max(round($totalBbl * $pricePerBbl * $pct / 100.0, 2), round($fixed, 2));
        if ($deposit < FinancialTransactionService::MIN_AMOUNT) {
            return 0.0;
        }
        if ((float)($terms['deposit_reputation_discount_enabled']['value'] ?? 0.0) <= 0.0) {
            return $deposit;
        }
        $modifier = 1.0;
        if ($contractReputation >= 80) {
            $modifier = 0.70;
        } elseif ($contractReputation >= 60) {
            $modifier = 0.85;
        } elseif ($contractReputation < 30) {
            $modifier = 1.25;
        }
        return round($deposit * $modifier, 2);
    }
     ...
     * @param array<string,mixed> $contract
     */
    private function calculateCancelPenalty(array $terms, array $contract, float $marketPrice): float
    {
        $totalBbl = (float)($contract['total_bbl'] ?? 0.0);
        $deliveredBbl = (float)($contract['delivered_bbl'] ?? 0.0);
        $remainingBbl = max(0.0, $totalBbl - $deliveredBbl);
        $pricePerBbl = $this->calculatePrice($terms, max(0.0, $marketPrice));
        $pct = max(0.0, (float)($terms['cancel_penalty_pct']['value'] ?? 0.0));
        $fixed = max(0.0, (float)($terms['cancel_penalty_fixed']['value'] ?? 0.0));
        $penalty = max(round($remainingBbl * $pricePerBbl * $pct / 100.0, 2), round($fixed, 2));
        return $penalty < FinancialTransactionService::MIN_AMOUNT ? 0.0 : $penalty;
    }

    /** @param array<string,mixed> $contract */
    private function settleSecurityDeposit(array $contract, string $newStatus, float $newDeliveredBbl, string $now): void
    {
        $contractId = (int)$contract['id'];
        $playerId = (int)$contract['player_id'];
        $deposit = round((float)($contract['security_deposit'] ?? 0.0), 2);
        $depositStatus = (string)($contract['security_deposit_status'] ?? 'none');
        if ($deposit < FinancialTransactionService::MIN_AMOUNT || $depositStatus !== 'paid') {
            return;
        }

        /** @var array<string,array{type:string,value:float,text:?string}> $terms */
        $terms = json_decode((string)($contract['terms_json'] ?? ''), true) ?? [];
        $refund = $this->securityDepositRefundAmount($terms, $newStatus, $deposit, (float)$contract['total_bbl'], $newDeliveredBbl);
        $status = $refund >= $deposit - 0.001 ? 'refunded' : ($refund >= FinancialTransactionService::MIN_AMOUNT ? 'partial_refund' : 'forfeited');

        if ($refund >= FinancialTransactionService::MIN_AMOUNT) {
            $this->fts ??= new FinancialTransactionService($this->db);
            ...
            if (!$credit['success']) {
                throw new RuntimeException('FTS deposit refund failed: ' . ($credit['error'] ?? 'unknown'));
            }
        }
        ...
        $this->db->prepare(
            "UPDATE player_contracts
                SET security_deposit_status = ?,
                    ...
                    updated_at = ?
              WHERE id = ? AND player_id = ?"
        )->execute([$status, $refund, $now, $contractId, $playerId]);

        $this->logEvent(
            $playerId,
            $contractId,
            (string)$contract['target_type'],
            $contract['target_id'] === null ? null : (int)$contract['target_id'],
            (string)$contract['context'],
            $refund >= FinancialTransactionService::MIN_AMOUNT ? 'contract_deposit_refunded' : 'contract_deposit_forfeited',
            $refund >= FinancialTransactionService::MIN_AMOUNT ? 'Contract deposit refunded' : 'Contract deposit forfeited',
            ['deposit' => $deposit, 'refund' => $refund, 'deposit_status' => $status]
        );
    }

    /** @param array<string,array{type:string,value:float,text:?string}> $terms */
    private function securityDepositRefundAmount(array $terms, string $newStatus, float $deposit, float $totalBbl, float $newDeliveredBbl): float
    {
        if ($newStatus === 'completed') {
        ...
        if ($newStatus === 'failed') {
            if ((float)($terms['deposit_partial_refund_enabled']['value'] ?? 0.0) > 0.0 && $totalBbl > 0.0 && $newDeliveredBbl > 0.0) {
                return round($deposit * max(0.0, min(1.0, $newDeliveredBbl / $totalBbl)), 2);
            }
            return (float)($terms['deposit_forfeit_on_fail']['value'] ?? 1.0) > 0.0 ? 0.0 : $deposit;
        }
        return 0.0;
    }

    private function currentMarketPrice(): float
    {
        try {
            $row = $this->db->query("SELECT current_price FROM market_state WHERE id = 1 LIMIT 1")->fetch(PDO::FETCH_ASSOC);
            return is_array($row) ? max(0.0, (float)$row['current_price']) : 70.0;
        } catch (Throwable $e) {
            if (class_exists('GameLog', false)) {
                GameLog::error('ContractService', 'currentMarketPrice FAILED', $e);
            }
            return 70.0;
        }
    }
