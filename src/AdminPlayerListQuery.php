...
final class AdminPlayerListQuery
{
     ...
     * @return array{filter: string, login_from: string, login_to: string, sort: string}
     */
    public static function filters(array $input): array
    {
        $result = [];
        foreach (['filter', 'login_from', 'login_to', 'sort'] as $key) {
        ...
        foreach (['login_from', 'login_to'] as $key) {
            $value = $result[$key];
            if ($value === '') {
                continue;
            }
            ...
            if (!$date || $date->format('Y-m-d') !== $value || $value < '1000-01-01' || $value > '9999-12-30') {
                throw new InvalidArgumentException('Invalid login date');
            }
        }
    ...
    public static function fetch(PDO $db, array $input): array
    {
        ...
        $stmt = $db->prepare("SELECT p.id, p.email, p.cash, p.status, p.last_login_at,
