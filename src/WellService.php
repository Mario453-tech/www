 ...
 */
    private function ensureSchema(): void
    {
        ...
        try {
            ...
            $allValues = array_merge($existing, $needed);
 ...
 // nieudane zapisy. / Repair wells stuck with an empty status from prior failed writes.
        ...
        // Add ticks_since_incident counter (immunity / pressure system)
        try {
            $col = $this->db->query("SHOW COLUMNS FROM `wells` LIKE 'ticks_since_incident'")->fetch();
            if (!$col) {
                $this->db->exec("ALTER TABLE `wells` ADD COLUMN `ticks_since_incident` INT NOT NULL DEFAULT 999 AFTER `post_incident_risk_boost`");
                if (class_exists('GameLog', false)) {
                    GameLog::info('WellService', 'wells.ticks_since_incident column added');
                }
            }
        } catch (Throwable $e) {
            if (class_exists('GameLog', false)) {
                GameLog::error('WellService', 'ensureSchema ticks_since_incident failed', $e);
            }
        }

        // Add service_prev_status (status frozen during a technical service task)
        try {
            $col = $this->db->query("SHOW COLUMNS FROM `wells` LIKE 'service_prev_status'")->fetch();
            if (!$col) {
                $this->db->exec("ALTER TABLE `wells` ADD COLUMN `service_prev_status` VARCHAR(32) DEFAULT NULL");
                if (class_exists('GameLog', false)) {
                    GameLog::info('WellService', 'wells.service_prev_status column added');
                }
            }
        } catch (Throwable $e) {
            if (class_exists('GameLog', false)) {
                GameLog::error('WellService', 'ensureSchema service_prev_status failed', $e);
            }
        }
    }
}
