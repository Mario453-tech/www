...
class ProtectionService
{
    ...
    /** Klucze efektow P2 hub / Effect keys P2 hub */
    public const EFFECT_KEYS_P2_HUB = [
        'equipment_damage_risk_mult',
        'local_leak_risk_mult',
        'critical_overload_risk_mult',
        'transfer_failure_risk_mult',
        'loading_error_risk_mult',
        'storage_jam_risk_mult',
    ];

    /** Klucze efektow P2 rurociag / Effect keys P2 pipeline */
    public const EFFECT_KEYS_P2_PIPELINE = ['pipeline_incident_risk_mult'];
    ...
    public function getAvailableOptions(int $playerId, string $targetType, string $context, float $referenceValue = 0.0): array
    {
        ...
        // Batch-fetch efektow jednym zapytaniem zamiast N+1.
        // Batch-fetch effects in a single query instead of N+1.
        $optionIds = array_map(static fn(array $o): int => (int)$o['id'], $options);
        $effectsMap = $this->effectsForMany($optionIds);
    ...
    {
        return $this->effectsForMany([$optionId])[$optionId] ?? [];
    }

    /**
     * Batch-fetch efektow dla wielu opcji jednym zapytaniem (unikamy N+1).
     * Batch-fetch effects for multiple options in a single query (avoids N+1).
     *
     * @param int[] $optionIds
    ...
    private function effectsForMany(array $optionIds): array
    {
        ...
        try {
            ...
            $stmt = $this->db->prepare(
                "SELECT protection_option_id, effect_key, effect_type, effect_value
                   FROM protection_effects WHERE protection_option_id IN ({$placeholders})"
            );
            $stmt->execute($optionIds);
            $out = [];
            foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
                $optId = (int)$row['protection_option_id'];
                $type  = (string)$row['effect_type'];
                $value = (float)$row['effect_value'];
                if ($type === 'mult') {
                    $value = max(0.05, min(1.0, $value));
                } else {
                    // delta: ogranicz do [-0.99, +0.99] zeby nie wychodzic poza zakres prawdopodobienstwa.
                    // delta: clamp to [-0.99, +0.99] to stay within probability range.
                    $value = max(-0.99, min(0.99, $value));
                }
                $out[$optId][(string)$row['effect_key']] = ['type' => $type, 'value' => $value];
            }
            return $out;
        } catch (Throwable $e) {
            GameLog::error('ProtectionService', 'effectsForMany FAILED', $e, ['option_ids' => $optionIds]);
            return [];
        }
    }
