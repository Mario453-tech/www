<?php
/**
 * HubApi.php AJAX endpoint dla moduu hubw logistycznych.
 ...
 * Gracz przypisuje swoje odwierty do istniejcych hubw nie buduje hubw samodzielnie.
...
try {
 ...
 // POST: upgrade hub (player-owned or admin)
        case 'upgrade_hub':
            $hubId = (int)($_POST['hub_id'] ?? 0);
            if ($hubId <= 0) hubApiOut(['success' => false, 'error' => t('common.validation_error')], 422);

            $result = $isAdmin
                ? $hubSvc->upgradeHub($hubId, $playerId)
                : $acqSvc->upgradeOwned($playerId, $hubId);

            if (!$result['success']) {
                $err = match($result['error'] ?? '') {
                    'max_level'          => t('logistics.hub.err_max_level'),
                    'insufficient_funds' => t('logistics.hub.err_insufficient_funds'),
                    'hub_not_owned'      => t('logistics.hub.err_hub_not_yours'),
            ...
            hubApiOut([
                'success'   => true,
                'message'   => t('logistics.hub.ok_upgrade', ['level' => $result['new_level'] ?? '?']),
        ...
        case 'buy_new_hub':
            ...
            if (!$result['success']) {
                $err = match($result['error'] ?? '') {
                    ...
                    'no_hub_permit'      => t('legal.hub.err_no_hub_permit'),
                    default              => t('logistics.hub.err_generic'),
                };
                hubApiOut(['success' => false, 'error' => $err]);
            }
