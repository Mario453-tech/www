...
trait LegalHubPermitTrait
{
    ...
    public function submitHubApplication(int $playerId, int $regionId, ?DateTimeInterface $now = null): array
    {
        ...
        try {
            ...
            // Audit trail (nieblokujacy) / Non-blocking audit trail
            try {
                if (class_exists('FinancialTransactionService', false)) {
                    (new FinancialTransactionService($this->db))->logTransaction(
                        ...
                        'Oplata za wniosek o zezwolenie na hub w regionie #' . $regionId
                    );
                }
        ...
        try {
            ...
            $this->db->prepare(
                "INSERT INTO director_notifications
                 ...
                 VALUES (?, 'legal_hub', ?, ?, ?, ?, 0, 'legal.php', 'Dział prawny — huby', ?)"
