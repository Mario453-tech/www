...
trait LegalBriberyTrait
{
    ...
    public function bribePermit(int $playerId, int $regionId): array
    {
        ...
        // Transitional + upgrade_pending=1: lapowka przyspiesza pelne zezwolenie.
        // Transitional + upgrade_pending=1: bribe speeds up the full-permit upgrade.
        $isTransitionalUpgrade = $status === self::STATUS_TRANSITIONAL
            && !empty($permit['application']['upgrade_pending']);

        if (in_array($status, self::ACTIVE_STATUSES, true) && !$isTransitionalUpgrade) {
            return ['success' => false, 'outcome' => 'error', 'message' => tPlain('legal.bribe.err_already_active')];
        }
        if (!in_array($status, self::BRIBE_ELIGIBLE_STATUSES, true) && !$isTransitionalUpgrade) {
            return ['success' => false, 'outcome' => 'error', 'message' => tPlain('legal.bribe.err_not_eligible')];
        }

        // Blokada po wpadce: gracz nie moze probowac ponownie do czasu wygasniecia kary.
        // Post-catch lock: the player cannot attempt again until the penalty expires.
        $lockUntil = $permit['application']['bribe_locked_until'] ?? null;
        if ($lockUntil !== null && new DateTime((string)$lockUntil) > new DateTime()) {
            return ['success' => false, 'outcome' => 'error', 'message' => tPlain('legal.bribe.err_locked')];
        }
        ...
        return $bribery->attempt(
            ...
            'legal_permit',
            $applicationCost,
            // Sukces: nadaj zezwolenie, wyczysc cooldown, termin decyzji i stan upgrade.
            // Success: grant the permit, clear cooldown, decision deadline and upgrade state.
                ...
                $db->prepare(
                    "UPDATE drilling_permit_applications
                        SET status='granted', decided_at=NOW(),
                            ...
                            upgrade_pending=0, upgrade_decision_due_at=NULL
            ...
            [
                ...
                'on_caught' => static function () use ($db, $playerId, $regionId, $caughtCooldownMinutes, $isTransitionalUpgrade): void {
                    $until = (new DateTime())->modify('+' . $caughtCooldownMinutes . ' minutes')->format('Y-m-d H:i:s');
                    if ($isTransitionalUpgrade) {
                        // Wpadka przy upgrade transitional: kasuj upgrade, blokuj lapowke,
                        // ale zachowaj status transitional (gracz nie traci dostepu).
                        // Caught on transitional upgrade: cancel upgrade, lock bribe,
                        // but keep transitional status (player does not lose access).
                        $db->prepare(
                            "UPDATE drilling_permit_applications
                        ...
                        $db->prepare(
                            "UPDATE drilling_permit_applications
                                SET status='refused', decided_at=NOW(), decision_due_at=NULL,
                                    ...
                                    bribe_locked_until=?
                              WHERE player_id=? AND region_id=?"
                        )->execute([$until, $until, $playerId, $regionId]);
                    }
                },
                'meta' => [
                    'label'        => $regionName,
