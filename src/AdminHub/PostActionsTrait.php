 ...
 */
 ...
 // Zapisz konfiguracje runtime obsady hubow.
            ...
            // Idempotent: writes only keys missing in DB. Leaves manual values intact.
            case 'seed_hub_type_defaults':
                $defaults = [
                    'small' => [
                        'slot_limit' => 2, 'nominal_bph' => 500, 'buffer_bbl' => 2000,
                        'opex_per_tick' => 500, 'build_cost' => 500000,
                        'repair_cost_pct' => 0.40, 'wear_per_tick' => 0.05,
                        'overload_wear_mult' => 3.0, 'overload_risk_mult' => 2.5,
                        'upgrade_cost' => 250000, 'max_level' => 3,
                    ],
                    'medium' => [
                        'slot_limit' => 5, 'nominal_bph' => 1500, 'buffer_bbl' => 6000,
                        'opex_per_tick' => 1200, 'build_cost' => 2000000,
                        'repair_cost_pct' => 0.40, 'wear_per_tick' => 0.05,
                        'overload_wear_mult' => 3.0, 'overload_risk_mult' => 2.5,
                        'upgrade_cost' => 1000000, 'max_level' => 3,
                    ],
                    'large' => [
                        'slot_limit' => 10, 'nominal_bph' => 3500, 'buffer_bbl' => 14000,
                        'opex_per_tick' => 2500, 'build_cost' => 8000000,
                        'repair_cost_pct' => 0.40, 'wear_per_tick' => 0.05,
                        'overload_wear_mult' => 3.0, 'overload_risk_mult' => 2.5,
                        'upgrade_cost' => 4000000, 'max_level' => 3,
                    ],
                ];
                ...
                try {
                    $rows = $db->query(
                        "SELECT config_key FROM logistics_hub_config WHERE config_group = 'hub_type'"
                    )->fetchAll(PDO::FETCH_COLUMN);
                    $existing = array_flip($rows);
                } catch (Throwable $e) {
                    GameLog::error('AdminHub', 'seed_hub_type_defaults: read existing FAILED', $e);
                }

                $inserted = 0;
                foreach ($defaults as $type => $kv) {
                    foreach ($kv as $k => $v) {
                        $fullKey = "{$type}.{$k}";
                        if (isset($existing[$fullKey])) {
                            continue;
                        }
                        if ($hubSvc->saveConfigValue('hub_type', $fullKey, 'global', (string)$v)) {
                            $inserted++;
                        }
                    }
                }

                AdminLog::log('hub_type_seed', "Seed hub_type defaults: {$inserted} kluczy");
                $msg = t('admin.logistics.cfg_seed_done', ['n' => $inserted]);
                $err = false;
                break;
        }
