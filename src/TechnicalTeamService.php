...
class TechnicalTeamService
{
 ...
 /** @return list<string> */
    ...
    /** @var array<int,bool> strażnik per połączenie (raz na proces, ponownie dla nowego PDO w testach) */
    private static array $hubTaskSchemaEnsured = [];

    private function ensureHubTaskSchema(): void
    {
        $schemaConnId = spl_object_id($this->db);
        if (isset(self::$hubTaskSchemaEnsured[$schemaConnId])) {
            return;
        }
        self::$hubTaskSchemaEnsured[$schemaConnId] = true;
