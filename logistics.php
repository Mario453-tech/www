...
// Huby sa infrastruktura systemowa - gracz przypisuje odwierty do istniejacych hubow | Hubs are system infrastructure - player assigns wells to existing hubs
 ...
 // Dostawy morskie gracza / Player marine deliveries
    ...
    // Ochrona: transport drogowy, huby, rurociagi / Protection: road, hubs, pipelines
    $roadProtectionWells      = [];
    ...
    try {
        ...
        // referenceValue = wartosc ladunku ciezarowkowego: suma(bbl/h) * cena_ropy
        // referenceValue = truck cargo value: sum(bbl/h) * current oil price
        $roadWellIds    = [];
        $roadTotalBblH  = 0.0;
        foreach ($wells as $wellRow) {
            if (($wellRow['transport'] ?? '') === 'ciezarowki') {
                $roadWellIds[]   = (int)$wellRow['id'];
                $roadTotalBblH  += (float)($wellRow['transported'] ?? 0.0);
            }
        }
        $roadRefValue = 0.0;
        if ($roadWellIds !== [] && $roadTotalBblH > 0.0) {
            try {
                $oilPriceRow  = $db->query("SELECT current_price FROM market_state WHERE id = 1")->fetch(PDO::FETCH_ASSOC);
                $roadRefValue = $roadTotalBblH * (float)($oilPriceRow['current_price'] ?? 0.0);
            } catch (Throwable) {
                $roadRefValue = 0.0;
            }
        }
        if ($roadWellIds !== []) {
            foreach ($protSvc->getAvailableOptions($playerId, 'road_transport', 'road_transport_guard', $roadRefValue) as $opt) {
                $opt['effect_lines'] = $protEffectLines($opt['effects']);
                $roadProtectionOptions[] = $opt;
            }
            $phs = implode(',', array_fill(0, count($roadWellIds), '?'));
            $nameStmt = $db->prepare("SELECT id, COALESCE(NULLIF(name,''), location_name) AS well_name FROM wells WHERE id IN ({$phs})");
            $nameStmt->execute($roadWellIds);
            $wellNames = $nameStmt->fetchAll(PDO::FETCH_KEY_PAIR);
            $activeRoadProt = $protSvc->getActiveProtections($playerId, 'road_transport', $roadWellIds, 'road_transport_guard');
            foreach ($roadWellIds as $rwId) {
                $ap = $activeRoadProt[$rwId] ?? null;
                $roadProtectionWells[] = [
                    'id'     => $rwId,
                    'name'   => (string)($wellNames[$rwId] ?? ''),
                    'active' => $ap === null ? null : ['name' => (string)$ap['option_name'], 'ends_at' => (string)$ap['ends_at']],
                ];
            }
        }

        // -- Huby: ochrona dla hubow nalezacych do gracza (player_id = playerId)
        // -- Hubs: protection for hubs owned by player (player_id = playerId)
        ...
        foreach ($hubCards as $card) {
            $hub   = $card['hub'] ?? [];
            $hubId = (int)($hub['id'] ?? 0);
            if ($hubId > 0 && (int)($hub['player_id'] ?? 0) === $playerId) {
                $ownedHubIds[] = $hubId;
            }
        }
        if ($ownedHubIds !== []) {
            $activeHubProt = $protSvc->getActiveProtections($playerId, 'hub', $ownedHubIds, 'hub_guard');
            foreach ($hubCards as $card) {
                $hub   = $card['hub'] ?? [];
                $hubId = (int)($hub['id'] ?? 0);
                if ($hubId <= 0 || (int)($hub['player_id'] ?? 0) !== $playerId) {
        ...
        if ($pipelines !== []) {
            ...
            foreach ($pipelines as $pipe) {
                $pid = (int)($pipe['id'] ?? 0);
                if ($pid > 0 && ($pipe['status'] ?? '') !== 'building') {
                    $pipeIds[] = $pid;
                }
            }
            if ($pipeIds !== []) {
                $activePipeProt = $protSvc->getActiveProtections($playerId, 'pipeline', $pipeIds, 'pipeline_guard');
                foreach ($pipelines as $pipe) {
                    $pid = (int)($pipe['id'] ?? 0);
                    if ($pid <= 0 || ($pipe['status'] ?? '') === 'building') {
                        continue;
                    }
                    $ap = $activePipeProt[$pid] ?? null;
                    $pipelineProtectionTargets[] = [
                        'id'     => $pid,
                        'name'   => tPlain('protection.target_pipeline', ['id' => $pid]),
                        'active' => $ap === null ? null : ['name' => (string)$ap['option_name'], 'ends_at' => (string)$ap['ends_at']],
                    ];
                }
            }
        ...
        GameLog::error('logistics.php', 'Protection data load failed', $e, ['player' => $playerId]);
    }
 ...
 // Ochrona / Protection
        'roadProtectionWells'       => $roadProtectionWells,
        'roadProtectionOptions'     => $roadProtectionOptions,
        'hubProtectionTargets'      => $hubProtectionTargets,
        'hubProtectionOptions'      => $hubProtectionOptions,
        'pipelineProtectionTargets' => $pipelineProtectionTargets,
        'pipelineProtectionOptions' => $pipelineProtectionOptions,
    ]);
