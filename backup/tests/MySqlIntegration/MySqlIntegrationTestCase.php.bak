...
abstract class MySqlIntegrationTestCase extends TestCase
{
    ...
    private function ensureTechnicalManagerExists(): void
    {
        ...
        if ($roleId === false) {
            $this->db->prepare(
                "INSERT INTO board_roles (id, code, name, slots_limit, created_at) VALUES (?, 'technical', 'Technical', 1, NOW())"
            )->execute([$ids['roleId']]);
            $roleId = $ids['roleId'];
        }
        ...
        if ($existsStmt->fetchColumn() === false) {
            $this->db->prepare(
                'INSERT INTO board_members (id, role_id, status, skill_organization) VALUES (?, ?, \'active\', 5)'
