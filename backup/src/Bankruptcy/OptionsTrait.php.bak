...
trait BankruptcyOptionsTrait
{
    ...
    private function applyBankTakeover(): array
    {
        ...
        $this->db->beginTransaction();
        try {
            $this->db->prepare("UPDATE wells SET status='seized' WHERE id=? AND player_id=?")->execute([$well['id'], $this->playerId]);
            $this->db->prepare("UPDATE loans SET remaining_amount = GREATEST(0, remaining_amount - ?), status = CASE WHEN GREATEST(0, remaining_amount - ?) = 0 THEN 'paid_off' ELSE status END WHERE id=?")
                ->execute([$debtReduction, $debtReduction, $loanId]);
        ...
        try {
            if ($directDebtRepay > 0) {
                ...
                foreach ($lateLoans->fetchAll() as $loan) {
                    ...
                    $this->db->prepare("UPDATE loans SET remaining_amount=GREATEST(0,remaining_amount-?), status=CASE WHEN GREATEST(0,remaining_amount-?)<=0 THEN 'paid_off' ELSE 'active' END, late_since=NULL WHERE id=?")->execute([$repay, $repay, $loan['id']]);
                    $remaining -= $repay;
                }
            }
