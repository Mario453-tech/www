...
trait IncidentMessagesTrait
{
    ...
    private const MSG_COUNT = [
        ...
        'medium' => ['operator' => 2, 'technician' => 2, 'hse' => 1],
    ...
    private function generateIncident(
        ...
        $opSkill   = $staffData['operator_skill']   ?? null;
        ...
        if (!$hseActive)         $causeWeights['hse']        = 15;
        elseif ($level === 'major' || $level === 'medium')
                                 $causeWeights['hse']        = 5;
        ...
        if ($causeCount !== null) {
            $msgKey = sprintf('incident.%s.%s.%d', $level, $cause, mt_rand(0, $causeCount - 1));
        } elseif ($systemCount > 0) {
            $msgKey = sprintf('incident.%s.system.%d', $level, mt_rand(0, $systemCount - 1));
        } else {
            $msgKey = 'incident.fallback_message';
        }

        $message = tPlain($msgKey, [
            'well'  => $wellId,
            'level' => $level,
            'drop'  => $drop,
            'hours' => $hours,
        ...
        return [
            ...
            'cost'        => $cost,
            'risk_add'    => $risk,
            'auto_repair' => $cfg['auto_repair'] ? 1 : 0,
            'hse_active'  => $hseActive ? 1 : 0,
            'message'     => $message,
        ];
    }

    private function getWorkerName(string $cause, array $staffData): string
    {
        $key = match($cause) {
        ...
        return ($key && !empty($staffData[$key])) ? $staffData[$key] : t('incident.unknown_worker');
    }
