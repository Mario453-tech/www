...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_settings' && $settingsExist) {
        ...
        $keys = [
            'apr_multiplier', 'risk_tolerance_modifier', 'max_amount_multiplier',
            'score_threshold_reject', 'score_threshold_cond', 'score_threshold_full',
            'apr_tier_conditional', 'apr_tier_partial', 'apr_tier_full',
        ];
        $limits = [
            'apr_multiplier'          => [0.1,  5.0],
            'risk_tolerance_modifier' => [0.1,  5.0],
            'max_amount_multiplier'   => [0.1,  5.0],
            'score_threshold_reject'  => [0,    60],
            'score_threshold_cond'    => [10,   80],
            'score_threshold_full'    => [30,   100],
            'apr_tier_conditional'    => [5.0,  80.0],
            'apr_tier_partial'        => [5.0,  60.0],
            'apr_tier_full'           => [1.0,  40.0],
        ];
        foreach ($keys as $key) {
            if (isset($_POST[$key])) {
                [$lo, $hi] = $limits[$key] ?? [0.1, 5.0];
                $val = round((float)$_POST[$key], 4);
                $val = max($lo, min($hi, $val));
                $old = BankSettings::get($key);
                $settings->set($key, $val, $adminUser);
                AdminLog::log('bank_settings_change', BankSettings::label($key) . ": {$old}  {$val}", null, 'system');
            }
        }
    ...
    elseif (!$needsMigration && $tablesExist) {
        ...
        elseif ($action === 'admin_reject' && $appId) {
            ...
            AdminLog::log('loan_admin_reject', "Override reject #{$appId}: {$reason}", null, 'system', $appId);
        ...
        elseif ($action === 'close_bailiff' && $procId) {
            ...
            $proc = $row->fetch();
            if ($proc) {
                $db->prepare("UPDATE bailiff_proceedings SET status='completed', completed_at=NOW() WHERE id=:id")->execute([':id'=>$procId]);
                $db->prepare("UPDATE loans SET status='active', late_since=NULL WHERE id=:id")->execute([':id'=>$proc['loan_id']]);
        ...
        elseif ($action === 'bailiff_advance' && $procId) {
            ...
            if ($proc && $proc['stage'] < 4) {
                ...
                AdminLog::log('bailiff_advance', "Advanced bailiff stage {$proc['stage']}{$newStage} for proceeding #{$procId}", $proc['player_id'], 'system', $procId);
        ...
        elseif ($action === 'bailiff_bankruptcy' && $procId) {
            ...
            $proc = $row->fetch();
            if ($proc) {
                $reason = trim($_POST['bankruptcy_reason'] ?? 'Wymuszone przez admina');
                $db->prepare("UPDATE players SET status='bankrupt', bankruptcy_at=NOW() WHERE id=:id")->execute([':id'=>$proc['player_id']]);
                $db->prepare("UPDATE loans SET status='defaulted' WHERE id=:id")->execute([':id'=>$proc['loan_id']]);
                $db->prepare("UPDATE bailiff_proceedings SET status='bankruptcy', completed_at=NOW() WHERE id=:id")->execute([':id'=>$procId]);
                AdminLog::log('forced_bankruptcy', "Forced bankruptcy by admin: {$reason}", $proc['player_id'], 'system', $procId);
...
if ($tablesExist) {
    ...
    $rse = new RiskScoreEngine();
    foreach ($applications as $a) {
        if ($a['status'] === 'pending') {
            try {
                $liveScores[(int)$a['player_id']] = $rse->calculateRiskScore((int)$a['player_id']);
            } catch (Throwable $e) {
                GameLog::warn('admin/loans.php', 'RiskScore failed for player', ['player_id' => $a['player_id']]);
            }
        }
    }
}

$riskSectionDescriptions = [
    'wells'        => ['label' => t('risk.section_wells'),        'desc' => t('risk.section_wells_desc'),        'max' => 20],
    'production'   => ['label' => t('risk.section_production'),   'desc' => t('risk.section_production_desc'),   'max' => 10],
    'storage'      => ['label' => t('risk.section_storage'),      'desc' => t('risk.section_storage_desc'),      'max' => 10],
    'cash'         => ['label' => t('risk.section_cash'),         'desc' => t('risk.section_cash_desc'),         'max' => 15],
    'behavior'     => ['label' => t('risk.section_behavior'),     'desc' => t('risk.section_behavior_desc'),     'max' => 15],
    'market'       => ['label' => t('risk.section_market'),       'desc' => t('risk.section_market_desc'),       'max' => 15],
    'history'      => ['label' => t('risk.section_history'),      'desc' => t('risk.section_history_desc'),      'max' => 20],
...
$viewData = [
    ...
    'settingDescs'   => [
        ...
        'score_threshold_reject'  => ['min'=>0,   'max'=>60,   'step'=>1,    'hint'=> t('admin.loans.hint_score_reject')],
        'score_threshold_cond'    => ['min'=>10,  'max'=>80,   'step'=>1,    'hint'=> t('admin.loans.hint_score_cond')],
        'score_threshold_full'    => ['min'=>30,  'max'=>100,  'step'=>1,    'hint'=> t('admin.loans.hint_score_full')],
        'apr_tier_conditional'    => ['min'=>5.0, 'max'=>80.0, 'step'=>0.5,  'hint'=> t('admin.loans.hint_apr_conditional')],
        'apr_tier_partial'        => ['min'=>5.0, 'max'=>60.0, 'step'=>0.5,  'hint'=> t('admin.loans.hint_apr_partial')],
        'apr_tier_full'           => ['min'=>1.0, 'max'=>40.0, 'step'=>0.5,  'hint'=> t('admin.loans.hint_apr_full')],
    ],
    'sysStatus'      => $sysStatus,
