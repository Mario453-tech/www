...
async function wsOpenAssign(wellId, role, wellName) {
    ...
    document.getElementById('ws-modal-title').textContent = wsl('modal_title_assign', { role: roleLabel });
    ...
    document.getElementById('ws-modal').classList.add('ws-open');
    ...
    try {
        ...
        if (staff.length === 0) {
            const roleReqs = role === 'operator'
                ? wsl('req_specs_operator')
                : wsl('req_specs_technician');
            document.getElementById('ws-modal-body').innerHTML =
        ...
        const available = staff.filter(s => !s.assigned_well_id);
        ...
        const renderCard = (s) => `<div class="ws-staff-card" onclick="wsAssign(${s.id})">
                <div class="ws-staff-info">
                    <div class="ws-staff-name">${s.first_name} ${s.last_name}</div>
                    <div class="ws-staff-meta">${s.spec_name}</div>
                </div>
        ...
        document.getElementById('ws-modal-body').innerHTML =
            '<div class="ws-section-label">' + wsl('section_available') + '</div>'
        ...
        overlay.innerHTML = `
            <div class="modal-box modal-box--danger">
                ...
                <div class="modal-actions">
                    <button class="modal-btn modal-btn--cancel" id="ws-confirm-cancel">${wsl('btn_cancel')}</button>
                    <button class="modal-btn modal-btn--danger" id="ws-confirm-ok">${wsl('btn_detach')}</button>
                </div>
            </div>`;
...
function wsCloseModal() {
    document.getElementById('ws-modal').classList.remove('ws-open');
