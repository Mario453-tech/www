...
<p style="color:#eb5757"><strong>USU� TEN PLIK PO DIAGNOSTYCE!</strong></p>
...
<tr><td>upload_tmp_dir</td><td><?= htmlspecialchars(ini_get('upload_tmp_dir') ?: '(domy�lny)') ?></td></tr>
...
<table>
<tr><th colspan="2">Katalog avatar�w: <?= htmlspecialchars($avatarDir) ?></th></tr>
...
if ($wrote !== false) {
    @unlink($testFile);
    echo '<p class="ok"> Zapis do katalogu dzia�a</p>';
} else {
    echo '<p class="err"> Zapis niemo�liwy: ' . htmlspecialchars($lastErr['message'] ?? 'nieznany b��d') . '</p>';
}
?>
...
echo '<tr><td>error</td><td>' . $f['error'] . ($f['error'] === 0 ? ' <span class="ok">(OK)</span>' : ' <span class="err">(B��D)</span>') . '</td></tr>';
...
$e  = error_get_last();
echo '<tr><td>move_uploaded_file</td><td>' . ($ok ? '<span class="ok"> SUKCES!</span>' : '<span class="err"> B��D: ' . htmlspecialchars($e['message'] ?? '?') . '</span>') . '</td></tr>';
if ($ok) { @unlink($dest); echo '<tr><td>Plik testowy</td><td><span class="ok">usuni�ty</span></td></tr>'; }
echo '</table>';
?>
<?php endif; ?>
