...
$pid = (int)($_GET['pid'] ?? 0);
if ($pid <= 0) { echo json_encode(['error' => tPlain('admin.bank_status.err_no_pid')]); exit; }
...
try {
    ...
    if (!in_array('remaining_amount', $cols)) {
        echo json_encode(['error' => tPlain('admin.bank_status.err_old_schema')]); exit;
    }
} catch (PDOException $e) {
    echo json_encode(['error' => tPlain('admin.bank_status.err_no_table')]); exit;
}
