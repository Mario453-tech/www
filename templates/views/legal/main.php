...
<section class="card">
    ...
    <?php foreach ($active as $entry): ?>
        ...
        <div class="legal-region-card legal-region-card--active">
            ...
            <?php endif ?>
            <p class="legal-region-note"><?= t('legal.unlocks_note') ?></p>
        </div>
    <?php endforeach ?>
    </div>
</section>
...
<!-- Regiony zablokowane kapitałowo (§7.3) / Capital-locked regions -->
...
<section class="card">
    ...
    <?php foreach ($capitalLocked as $entry): ?>
        ...
        $missing = max(0.0, $requiredCapital - $cash);
        ?>
        <div class="legal-region-card legal-region-card--capital-locked">
            ...
            <div class="legal-region-meta">
                <span><?= t('legal.risk_label') ?>: <span class="legal-risk-<?= htmlspecialchars($cfg['risk_level']) ?>"><?= t('legal.risk.' . $cfg['risk_level']) ?></span></span>
                <span><?= t('legal.required_capital_label') ?>: <?= number_format($requiredCapital, 0, ',', ' ') ?> PLN</span>
            </div>
            <div class="legal-capital-missing">
                <?= t('legal.capital_missing', ['amount' => number_format($missing, 0, ',', ' ')]) ?>
            </div>
        </div>
    <?php endforeach ?>
    </div>
