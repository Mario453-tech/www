...
<script>
(function () {
    var m = <?= json_encode($msg, JSON_UNESCAPED_UNICODE) ?>;
    var t = <?= json_encode($msgType, JSON_UNESCAPED_UNICODE) ?>;
    function show() {
        if (t === 'error' && typeof window.alertError === 'function') {
            window.alertError(m);
        } else if (typeof window.alertInfo === 'function') {
            window.alertInfo(m);
        } else if (typeof window.showGameToast === 'function') {
            window.showGameToast(m, t === 'error' ? 'error' : 'success');
        }
    }
...
window.TECH_LANG = <?= json_encode([
    ...
    'strike_blocked_message'  => t('technical.task_msg.staff_on_strike'),
    'strike_blocked_title'    => t('technical.strike_blocked_title'),
    'strike_conflicts_label'  => t('technical.strike_conflicts_label'),
    'strike_close_label'      => t('common.close'),
    'strike_conflicts_url'    => url('hr', ['tab' => 'conflicts']),
    'repair_no_mnt'           => t('technical.repair_no_mnt'),
    'repair_no_mnt_title'     => t('technical.repair_no_mnt_title'),
    'repair_mnt_busy'         => t('technical.repair_mnt_busy'),
