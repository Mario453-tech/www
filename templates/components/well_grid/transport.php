...
?>
 ...
 // Rurociag odwiert->hub kupiony i operacyjny -> stan aktywny (zielona ramka + "Aktywny")
                            $isActivePipe = ($tCode === 'rurociag' && !$w['_isOffshore'] && !empty($w['_pipelineOperational']));
                            ...
                            $__cardCls = ($isCurrent || $isActivePipe) ? 'wg-opt-card--active' : ($isDisabled ? 'wg-opt-card--disabled' : '');
                        ?>
                            <div class="wg-opt-card <?= $__cardCls ?>">
                                <div class="wg-opt-hdr">
                                    <span class="wg-opt-name"><?= $tName ?></span>
                                    <?php if ($isActivePipe): ?>
                                    <span class="wg-opt-badge wg-opt-badge--ok">&#9679; <?= t('well_grid.pipeline_active') ?></span>
 ...
 // Second transport leg: hub -> storage / Odcinek 2: hub -> magazyn
                        ...
                        $__outOperational = !empty($w['_outboundPipelineOperational']);
 // Rurociag hub->magazyn kupiony, dziala i nie jest w budowie -> stan aktywny (zielona ramka)
                        $__outActive      = $__outType === 'rurociag' && $__outOwned && $__outOperational && !$__outBuilding;
                        ...
                        ?>
                        <div class="wg-leg2<?= $__outActive ? ' wg-leg2--active' : '' ?>">
                            <div class="wg-leg2-hdr">
                                ...
                                <?php if ($__outActive): ?>
                                <span class="wg-leg2-active-badge">&#9679; <?= t('well_grid.leg2_active') ?></span>
                                <?php endif ?>
                            </div>
