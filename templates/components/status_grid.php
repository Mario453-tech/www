...
<?php foreach ($statusItems as $item):
    ...
    $walletFmt  = $item['data_wallet_fmt'] ?? 'dec';
    $walletAttr = !empty($item['data_wallet_key'])
        ? ' data-wallet-' . htmlspecialchars($item['data_wallet_key'], ENT_QUOTES, 'UTF-8') . ' data-wallet-fmt="' . htmlspecialchars($walletFmt, ENT_QUOTES, 'UTF-8') . '"'
        : '';
?>
